...
namespace CP
{
  ...
  public:
    ...
    void reorder(int pos, std::vector<int> selected)
    {
      ...
      iterator pos_it = begin();
      for (int i = 0; i < pos; i++)
        pos_it++;

      std::vector<iterator> to_insert(selected.size());
      for (size_t i = 0; i < selected.size(); i++)
      {
        iterator selected_it = begin();
        for (size_t j = 0; j < selected[i] ; j++)
        {
          selected_it++;
        }
        to_insert[i] = selected_it;
      }
      for (iterator &insert_it : to_insert)
      {
        insert(pos_it, *insert_it);
        erase(insert_it);
      }
